...
export default function LogIn({ setLogIn }) {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    //some code to fetch from backend/database
    const data = {
      username: email,
      password: password,
    };
    const request = await fetch("http://localhost:3000/loginreq", {
      method: "POST",
      headers: {
          ...
          <div className="flex flex-col justify-center items-center gap-4">
            ...
            >
              ...
              <input
                ...
                className="input input-bordered w-full max-w-xs  text-black border-[#D1D5DB] bg-[#F3F4F6]"
              />
              <button className="btn bg-[#0166FF] w-[325px] text-[#FFFFFF] rounded-3xl">
                Log in
              </button>
            </form>
          </div>
